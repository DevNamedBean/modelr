...
function duplicateSelected(exactPosition = false) { if (!selected) return; rememberScene(); const type = selected.name.startsWith('Sphere') ? 'Sphere' : selected.name.startsWith('Cylinder') ? 'Cylinder' : selected.name.startsWith('Torus') ? 'Torus' : selected.name.startsWith('Cone') ? 'Cone' : selected.name.startsWith('Crown') ? 'Crown' : 'Cube'; const position = exactPosition ? selected.position.toArray() : [selected.position.x + .6, selected.position.y, selected.position.z + .6]; const duplicate = addObject(type, selected.material.color.getHex(), position); duplicate.geometry.dispose(); duplicate.geometry = selected.geometry.clone(); duplicate.material.dispose(); duplicate.material = selected.material.clone(); duplicate.scale.copy(selected.scale); duplicate.rotation.copy(selected.rotation); duplicate.userData = { ...selected.userData }; selectObject(duplicate); }
