...
window.addEventListener('keyup', event => cameraKeys.delete(event.key.toLowerCase()));
...
document.querySelector('#renamePart').onclick = () => { if (!contextPart) return; const name = window.prompt('Nieuwe naam voor deze part:', contextPart.name); if (name?.trim()) { contextPart.name = name.trim(); selectObject(contextPart); updateList(); } partContext.classList.remove('open'); };
