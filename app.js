...
function renderProjects() { const list = document.querySelector('#projectList'); const saved = projects(); list.innerHTML = saved.length ? saved.map(project => `<button class="project-row" data-project="${project.id}">◈ <span><b>${project.name}</b><small>Last update: ${project.updatedAt ? new Date(project.updatedAt).toLocaleString('nl-NL') : 'Onbekend'}</small></span><small>${project.objects} objects</small></button>`).join('') : '<div class="project-empty">Nog geen opgeslagen projecten.</div>'; list.querySelectorAll('.project-row').forEach(row => row.onclick = () => { const project = projects().find(item => item.id === row.dataset.project); if (!project) return; restoreScene(project.scene); currentProjectName = project.name; localStorage.setItem('modelrProjectName', currentProjectName); document.querySelector('#projectName').textContent = currentProjectName; projectModal.classList.remove('open'); }); }
...
document.querySelector('#confirmProjectModal').onclick = () => projectModalMode === 'confirm' ? openProjectModal('save') : projectModalMode === 'projects' ? projectModal.classList.remove('open') : saveProject();
